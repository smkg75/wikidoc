...
- The workspace is a local git repository with no remote, and it is the backup. A pass opens by committing whatever is uncommitted as `before pass <pass>` — that commit holds what sessions wrote since the last one — and closes at Learn with `pass <pass>`. An edit of `config.yaml` or of the ledger that worries you gets a commit before it, never a `.bak` beside it. No `.git` at the workspace root: `git init` it there, `.gitignore` `logs/`, `cache/`, `bench/`, `.trash/`, `.DS_Store`, `*.bak*`.
...
Done when `bench/` is gone from the workspace, every leftover is named in `wiki/state.md`, and the `pass <pass>` commit holds the ledger, the config and the wiki as the pass left them.
